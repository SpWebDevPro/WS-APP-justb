<?php
/**
 * Plugin Name: LatePoint Resea Addon - Google Calendar
 * Plugin URI: 
 * Description: LatePoint Resea addon for google calendar integration
 * Version:     1.0.0
 * Author:      LatePoint/SpWebDev
 * Author URI:  
 * Text Domain: latepoint-google-calendar
 * Domain Path: /languages
 */

 //based on Latepoint addon google calendar  Version:     1.0.4
...
class LatePointAddonGoogleCalendar {
  ...
  public function process_action_booking_status_changed($booking_id, $old_status){
    ...
    BookAppOsGoogleCalendarHelper::create_or_update_booking_in_gcal($booking_id);
  }

  public function process_action_booking_created($booking){
    /* OsGoogleCalendarHelper::create_or_update_booking_in_gcal($booking->id); */
    BookAppOsGoogleCalendarHelper::create_or_update_booking_in_gcal($booking->id);
  }
            ...
            <?php if(OsGoogleCalendarHelper::is_agent_connected_to_gcal($agent->id)){ ?>
              <div class="channel-watch-status watch-status-on">
                <div class="status-watch-label">
                  ...
                  <span class="cw-status"><?php _e('Google Calendar is Connected', 'latepoint-google-calendar'); ?></span>
                </div>
            ...
            <?php }else{ ?>
              <div class="channel-watch-status watch-status-off">
                ...
                <a href="#" class="os-google-cal-authorize-btn latepoint-link cw-enable" 
                  ...
                  <span><?php _e('Connect', 'latepoint-google-calendar');?> </span>
                </a>
              </div>
            <?php } ?>
					</div>
